  ...
  <div class="box box-primary">
    ...
    <div class="box-body">
      ...
      <div class="container-fluid">
        ...
        <div class="row">
          ...
          <div class="col col-md-6">
            <div class="container-fluid well">
              ...
              <br> &nbsp;&nbsp;&nbsp;&nbsp;
              {{  \Carbon\Carbon::parse($program->tarikh_mula)->format('d/m/Y') }} - {{ \Carbon\Carbon::parse($program->tarikh_akhir)->format('d/m/Y') }}
              ...
              <a href="{{ route('program.show', ['id' => $program->id_program]) }}" class="btn btn-info">Perincian</a>

              @role(['urusetia', 'superadmin'] && ! checkTarikhProgram($program))
              @if ($program->tempat == 7)
                    <a href="{{ route('program.join-zoom',['id' => $program->id_program]) }}" class="btn btn-primary" @if(!$agent->isDesktop()) target="_blank" @endif >Test Live Zoom</a>
              @endif
              @endrole
              ...
              @if ($program->senaraiPermohonanPesertaDiterima()->find(auth()->user()->id_pengguna))
                ...
                @if (checkTarikhProgram($program))
                  ...
                  @if ($program->tempat == 7)
                    <a href="{{ route('program.join-zoom',['id' => $program->id_program]) }}" class="btn btn-primary" @if(!$agent->isDesktop()) target="_blank" @endif >Join Live Zoom</a>
                  @endif
                      ...
                      <tr>
                        ...
                        <td>
                          @if (!$program->kehadiranSemasaProgram()->find(auth()->user()->id_pengguna))
                            Sila Dapatkan Link Kehadiran Daripada Urusetia Untuk Daftar Kehadiran
                          @else
                      ...
                      <tr>
                        ...
                        <td>
                          ...
                          @if (checkTarikhAkhirProgram($program))
                            @if (!$program->kehadiranSemasaProgram()->find(auth()->user()->id_pengguna))
                              <small class="label bg-yellow">Sila Daftar Kehadiran Dahulu</small>
                            @elseif ($program->senaraiPesertaTelahJawabKajiSelidik()->find(auth()->user()->id_pengguna))
                              <small class="label bg-green">Terima Kasih Telah Menjawab Kaji Selidik</small>
                            @else
                              <a href="{{ route('create.peserta.kajiselidik',['idProgram' => $program->id_program]) }}" class="btn btn-warning" >Jawab Kaji Selidik</a>
                            @endif
                          @else
                            <small class="label bg-yellow">Kaji Selidik Belum Dibuka</small>
                          @endif
                        ...
                        <td>
                          @if (checkTarikhAkhirProgram($program))
                            ...
                            @elseif ($program->senaraiPesertaBelumJawabKajiSelidik()->find(auth()->user()->id_pengguna))
                              <small class="label bg-yellow">Sila Jawab Kaji Selidik Dahulu</small>
                            @elseif ($program->senaraiPesertaTidakHadir()->find(auth()->user()->id_pengguna))
                              Sijil Tidak Dapat Dijana Kerana Anda Tidak Hadir Atau Hadir Sebahagian Hari Program
                            @else
                              <a href="{{ route('program.sijil',['url_sijil' => $program->senaraiPesertaTelahJawabKajiSelidik()->find(auth()->user()->id_pengguna)->senarai_peserta->url_sijil]) }}" class="btn btn-success" target="_blank">Muat Turun Sijil Penyertaan</a>
                            @endif
                          @else
              ...
              @else
                @if (!checkLepasTarikhProgram($program))
                  <a href="{{ route('mohon.peserta.individu', ['id' => $program->id_program]) }}" class="btn btn-primary pengesahan" onclick="return false">Mohon Sertai Program</a>
                @endif
              @endif
        ...
        @empty
        <p><h5>Tiada Program Dibuka</h5></p>
        @endforelse
      </div>
