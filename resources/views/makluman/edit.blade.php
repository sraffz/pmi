...
<div class="box box-primary">
  ...
  <div class="box-body">
      <form action="{{ route('makluman.update',['id' => $makluman->id]) }}" method="POST" class="col-lg-6" id="tempatForm">
          {{ csrf_field() }}
          ...
          <div class="form-group has-feedback {{ $errors->has('tajuk') ? 'has-error' : '' }}">
            ...
            <input type="text" name="tajuk" class="form-control"
                value="{{ old('tajuk', $makluman->tajuk) }}" />
        ...
        <div class="form-group has-feedback {{ $errors->has('keterangan') ? 'has-error' : '' }}">
            ...
            <textarea name="keterangan" id="keterangan" cols="30" rows="10" class="form-control">{{ old('keterangan', $makluman->keterangan) }}</textarea>
